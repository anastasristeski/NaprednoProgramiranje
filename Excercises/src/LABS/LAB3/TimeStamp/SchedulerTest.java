...
//        return this.getTime().equals(o.getTime());
//    }
...
class Scheduler<T>{
    ...
    public Timestamp<T> next(){
        ...
        timestamps.sort(Comparator.comparing(Timestamp::getTime));
        return timestamps.stream()
                .filter(x->x.getTime().isAfter(lt))
                .findFirst()
                .orElse(null);
    }
    public Timestamp<T> last(){
        ...
        timestamps.sort(Comparator.comparing(Timestamp::getTime));
        return timestamps.stream()
                .filter(x->x.getTime().isBefore(lt))
